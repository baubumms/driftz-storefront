...
export async function shopifyFetch(endpoint: string, tokens: {admin?: string, storefront?: string}, params: { query: Record<string, any>, variables: Record<string, any> }) {
  ...
  try {
    ...
    const payload = { query: params.query, variables: {...params.variables, locale: langCode?.toUpperCase()} };
